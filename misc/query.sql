...
,cnpj_dataset_unnest_cnae as
(
  select
    cnpj.* except (cnae_secundario)
    ,cnae as cnae_secundario
    ,row_number() over(partition by cnpj.cnpj) as row_number_cnpj
  from cnpj_dataset as cnpj
    left join cnpj.cnae_secundario as cnae
)

,cnpj_dataset_cnae_sec_desc as
(
  select
    t1.cnpj
    ,t1.row_number_cnpj
...
(
  select distinct
    ...
    ,data_info
  from cnpj_dataset_cnae_sec_desc t1
)
...
select
  ...
  ,t1.data_info
from cnpj_dataset_cnae_sec_desc_transposed t1
